...
def main(args):
    # 单 GPU 模式，确保至少有一个 GPU 可用
    assert torch.cuda.is_available(), "Training currently requires at least one GPU."
    device = 'cuda'
    rank = 0
    torch.manual_seed(args.global_seed)
    print(f"Starting single-GPU mode with global_seed={args.global_seed}.")

    # 设置特征存储文件夹
    os.makedirs(args.code_path, exist_ok=True)
    os.makedirs(os.path.join(args.code_path, 'code'), exist_ok=True)
    os.makedirs(os.path.join(args.code_path, 'image'), exist_ok=True)
    os.makedirs(os.path.join(args.code_path, 'control_depth'), exist_ok=True)
    os.makedirs(os.path.join(args.code_path, 'caption_emb'), exist_ok=True)
    ...
    if args.ten_crop:
        ...
        transform = transforms.Compose([
            ...
            transforms.TenCrop(args.image_size), # 返回一个包含多个 PIL 图像的元组
    ...
    for batch in tqdm(loader):
        ...
        train_steps = total  # 单 GPU 下，直接用 total
