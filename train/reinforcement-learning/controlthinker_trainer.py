...
import os
import sys
sys.path.append("/root/ControlAR")
# sys.path.append("/mnt/dolphinfs/hdd_pool/docker/user/hadoop-basecv/jiaoyang/jiaoyang/controlar")
from tokenizer.tokenizer_image.vq_model import VQ_models
from autoregressive.models.gpt_t2i_grpo import GPT_models
from safetensors.torch import load_file
from language.t5_grpo import T5Embedder
import torch
...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        def load_images_batch(images_tensor):
            
            transform = transforms.Compose([
                transforms.Resize((256, 256)), 
            ])
            
            transformed_images = torch.stack([transform(image) for image in images_tensor])
            
            transformed_images = (transformed_images + 1) / 2
            
            return transformed_images 

        self.clip_processor = CLIPProcessor.from_pretrained("/data/checkpoints/controlar/pickscore/laion/CLIP-ViT-H-14-laion2B-s32B-b79K", local_files_only=True)
        self.clip_model = CLIPModel.from_pretrained("/data/checkpoints/controlar/pickscore/laion/CLIP-ViT-H-14-laion2B-s32B-b79K", local_files_only=True).eval()
        ...
        def reward_format(completions, **kwargs):
            ...
            for each_completion in completions:
                reward = 1 if re.search(pattern, each_completion) else 0
                rewards.append(reward)

            return rewards

        def reward_no_truncation(prompts, clip_processor, device, **kwargs):
            rewards = []

            tokenized_no_trunc = clip_processor(
                text=prompts, 
                padding=False, 
                truncation=False, 
            ...
            for input_ids in tokenized_no_trunc['input_ids']:
                actual_length = len(input_ids)
                print(f"prompt token length for device {device} is {actual_length}")
                if actual_length > 82:
                    rewards.append(0)
                else:
                    rewards.append(1)
        ...
        def reward_clipscore(generated_imgs_pil, prompts, clip_processor, clip_model, device, **kwargs):
            import torch
            from itertools import islice
            from torch.nn.functional import cosine_similarity

            # 确保 prompts 是一个字符串或字符串列表
            ...
            assert len(generated_imgs_pil) == len(prompts)
            ...
            for img_batch, prompt_batch in zip(
                    batch_iterable(generated_imgs_pil, 6),
                    batch_iterable(prompts, 6)):
                
                length_scales = []
                prompt_batch_test = prompt_batch
                tokenized_no_trunc = clip_processor(
                    text=prompt_batch_test, 
                    padding=False, 
                    truncation=False, 
                    return_attention_mask=True
                )
                ...
                for input_ids in tokenized_no_trunc['input_ids']:
                    actual_length = len(input_ids)
                    length_scale = 0.5+0.5*(1 - math.cos(actual_length * math.pi / 67)) / 2
                    length_scales.append(length_scale)

                image_inputs = clip_processor(images=img_batch, return_tensors="pt").to(device)
                text_inputs = clip_processor(text=prompt_batch, padding=True, truncation=True, return_tensors="pt").to(device)
                ...
                batch_clip_scores = cosine_similarity(image_embs, text_embs, dim=-1)
                print("Original clip scores for device {}: {}".format(device, batch_clip_scores))
                print("Original length scale for device {}: {}".format(device, length_scales))
                for score_idx in range(len(batch_clip_scores)):
                    batch_clip_scores[score_idx] = batch_clip_scores[score_idx] * length_scales[score_idx]

                clip_scores_all.extend(batch_clip_scores.cpu().tolist())

            return clip_scores_all

        
        reward_funcs = [reward_format, reward_no_truncation, reward_clipscore]
        self.reward_funcs = reward_funcs


        # end reward definition
        ...
        # push reward model to gpu
        # self.clip_processor = self.accelerator.prepare_model(self.clip_processor, evaluation_mode=True)
    ...
    def _score_completions(self, inputs):
        ...
        # added input
        print(f"input['is_truncated']: {inputs[0]['is_truncated']}")
        control_img_paths = [example['images'][0]['path'] for example in inputs]
        print(f"control_img_paths: {control_img_paths}")
        gt_img_paths = [control_img_path.replace("dir_name_of_control_images", "image") for control_img_path in control_img_paths]
        print(f"gt_img_paths: {gt_img_paths}")
        gt_imgs = [Image.open(gt_img_path) for gt_img_path in gt_img_paths]

        # 1. extract original prompts from inputs
        ori_prompts = []
        for example in inputs:
            split_text = example['messages'][1]['content'].split("Original Prompt: ")
            if len(split_text) > 1:
                after_prompt_part = split_text[1]
                split2 = after_prompt_part.split("\n\nPlease analyze", 1)
                if len(split2) > 1:
                    original_prompt = split2[0].strip()
                    ori_prompts.append(original_prompt)
                    print(f"CUDA:{device.index} Extracted original prompt: {original_prompt}")
            else:
                print("Cannot find 'The original prompt:'")
                exit()

        generated_prompts = []
        answer_pattern = r"<answer>([\s\S]*?)</answer>"
        for each_completion in completions:
            match = re.search(answer_pattern, each_completion)
            if match:
                answer_text = match.group(1).strip().strip('\'"').strip()
                generated_prompts.append(answer_text)
                print(f"CUDA:{device.index} the extracted enhanced prompt is: {answer_text}")
            else:
                generated_prompts.append("")
                print(f"CUDA:{device.index} answer tag not contained in this completion")
        ...
        for i, (reward_func, reward_template) in enumerate(zip(self.reward_funcs, self.reward_templates)):
            ...
            # reward function
                ...
                # output_reward_func = reward_func(completions=completions, **reward_kwargs, **reward_inputs)
                output_reward_func = reward_func(completions=completions, **reward_inputs)
                rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)

        total_rewards_per_func = gather(rewards_per_func)
        print(f"total_rewards_per_func: {total_rewards_per_func}")
        print(f"reward_weights: {self.reward_weights}")
